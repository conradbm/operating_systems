...
// if you decide to interrupt the current thread and switch to another, how to handle it.
...
class Scheduler {
  ...
  static final int POLICY_PRIO_NP = 1; // Non preeminant
  static final int POLICY_PRIO_P = 2;  // preeminent
  ...
  public static InterruptHandler ih = new InterruptHandler();
  ...
  public static void run(NachosThread nextThread) {
    ...
    // TODO - Finish RR Schedule Thread Check.
    if(policy == POLICY_RR)
    {
    	if(nextThread.timeLeft > 4)
    	{
    		Interrupt.schedule(ih, 40, 0);
    	}
    }


    Debug.println('t', "Switching from thread: " + oldThread.getName() +" to thread: " + nextThread.getName());
    //System.out.println("Switching from thread: " + oldThread.getName() + " to thread: " + nextThread.getName());
    ...
    //System.out.println("Now in thread: " + NachosThread.thisThread().getName());
  ...
  public static boolean shouldISwitch(NachosThread current, NachosThread newThread)
  {
	//MP1 preemption code
	  ...
	  if(policy == POLICY_PRIO_P)
	  {
			 ...
			 {
				 System.out.println("Next thread's priority is NOT greater than current thread's");
			 }
	  }
	  
	  else if(policy == POLICY_SJF_P)
	  {
			 ...
			 {
				 System.out.println("Next thread's time left is NOT less than the current thread's time left.");
			 }
	  }
