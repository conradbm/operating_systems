...
class NachosThread extends Thread implements Printable {
  ...
  public void fork(Runnable runObj) {
	  // Print the parent of this function
	  Debug.print('t', "Forking thread: " + getName() + "\n");
	  Debug.ASSERT((status == JUST_CREATED), "Attempt to fork a thread that's already been forked");
	  
	  // Update parents runnable object. 
	  runnableObject = runObj;
	  
	  // Does this actually run the thread? Or does it just get shoved in the readyQueue?
	  start();
