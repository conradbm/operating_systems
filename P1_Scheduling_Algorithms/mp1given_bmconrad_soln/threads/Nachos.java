...
class Nachos implements Runnable {
  ...
  public void run() {
    ...
    {
      // timer = new Timer(new TimerInterruptHandler(), randomYield);
      SchedulerTest.ThreadTest(Scheduler.policy);
    }
	  ...
	  // for console input
      	...
      	if (FILESYS) {
      		if (args[i].equals("-cp")) {	// copy from UNIX to Nachos
      			Debug.ASSERT((i<args.length-2),"usage: -cp <filename1> <filename2>");
...
//----------------------------------------------------------------------
  ...
  public static void main(String clArgs[]) {
    ...
    Scheduler.setSchedulerPolicy(-1);
    for (int i=0; i<clArgs.length; i++)
    {
       if (clArgs[i].equals("-st"))
       {
    	   if (i+1 < clArgs.length)
    	   {
    		   SchedulerTest.initializeSchedulerType(clArgs[i+1]);
    		   break;
    	   }
       }
    }
    if (Scheduler.policy == -1)
    {
       System.out.println("Invalid Scheduler policy");
       System.exit(-1);
    }
    ...
    /*** FORKING A NACHOSTHREAD PUTS IT IN THE READY QUEUE FOR FCFS ALGOR ***/
    /*** YOU DECIDE HOW YOU WANT IT TO WORK IN THE OTHER ALGORS           ***/
    t.fork(new Nachos());
