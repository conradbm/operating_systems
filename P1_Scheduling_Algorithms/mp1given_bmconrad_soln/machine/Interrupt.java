...
class Interrupt {
 ...
 static {
  ...
  //----------------------------------------------------------------------
  public static PendingInterrupt schedule(Runnable handler, int fromNow, int type) 
  {
    int when = Nachos.stats.totalTicks + fromNow;
    PendingInterrupt toOccur = new PendingInterrupt(handler, when, type);
    Debug.printf('i', "Scheduling interrupt handler the %s at time = %d\n", intTypeNames[type], new Integer(when));
    System.out.println("Scheduling interrupt handler the" +  intTypeNames[type] +" at time = " + new Integer(when));
    Debug.ASSERT(fromNow > 0);
    
    // put a runnable into the interrupt handler.
  ...
  private static boolean checkIfDue(boolean advanceClock) {
    int old = status; // idle=0, kernel=1, usermode=2
    int when;

    Debug.ASSERT(level == IntOff);	// interrupts need to be disabled, to invoke an interrupt handler
